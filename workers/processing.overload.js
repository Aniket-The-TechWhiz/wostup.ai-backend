...
const User = require("./user.model"); // must have workingHoursPerDay field
...
function computeTaskWeight(task, today = new Date()) {
    // No fallbacks — this assumes estimatedEffort and priority are always
    // set on every task. If either is null, that's a data-entry gap to
    // fix upstream (e.g. require them at task-creation time), not
...
const personScoringWorker = new Worker(
    ...
    async (job) => {
        ...
        //    "active" now excludes done tasks; blocked/waiting-review
        //    are still included (just discounted in the weight calc).
        ...
        const workingHoursPerDay = person.workingHoursPerDay;
        ...
        // TODO: persist result (e.g. an OverloadScore collection),
        // and append a daily history snapshot for the persistence check.
        // await OverloadScore.updateOne({ workspaceId, userId }, result, { upsert: true });
